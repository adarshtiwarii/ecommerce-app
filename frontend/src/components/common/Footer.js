      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 mb-8">
        {[
          ...
          <div key={section.title}>
            <h3 className="text-gray-500 text-xs font-black uppercase mb-3">{section.title}</h3>
            <ul className="space-y-2">
              {section.links.map(link => <li key={link}><Link to="#" className="text-sm text-gray-700 hover:text-orange-500">{link}</Link></li>)}
            </ul>
          </div>
        ))}
      </div>
      <div className="border-t pt-5 flex flex-col lg:flex-row items-center justify-between gap-4 text-sm text-gray-600">
        <Link to="/" className="text-orange-500 font-black text-xl">EcoMart</Link>
