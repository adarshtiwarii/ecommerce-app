            ...
            <div className="flex h-44 items-end gap-3 border-b border-l px-3 pt-4">
              {(revenueBars.length ? revenueBars : [0, 0, 0, 0]).map((amount, index) => (
                <div key={index} className="flex flex-1 flex-col items-center gap-2">
                  <div className="w-full rounded-t-sm bg-[#FF6B00] transition-all" style={{ height: `${Math.max((amount / maxRevenueBar) * 130, 8)}px` }} />
            ...
            <div className="mt-4 space-y-3">
              {Object.entries(statusCounts).length === 0 ? <p className="text-sm text-white/50">No order data yet</p> : Object.entries(statusCounts).map(([status, count]) => (
                <div key={status}>
                  ...
                  <div className="h-2 rounded-full bg-[#0D0D0D]"><div className="h-2 rounded-full bg-[#FF6B00]" style={{ width: `${Math.min((count / Math.max(orders.length, 1)) * 100, 100)}%` }} /></div>
                </div>
              ))}
            </div>
          </div>
        ...
        {activePanel === 'products' && (
          <div className="space-y-4">
            <div className="bg-[#161616] border rounded-xl shadow-sm p-3 flex flex-col md:flex-row gap-3">
              ...
              <div className="flex border rounded-xl overflow-hidden">{[{value:'all',label:'All'},{value:'active',label:'Active'},{value:'disabled',label:'Disabled'}].map(opt => <button key={opt.value} onClick={() => setStatusFilter(opt.value)} className={`px-4 py-2 text-sm font-bold ${statusFilter === opt.value ? 'bg-[#FF6B00] text-white' : 'bg-[#161616] text-white/70 hover:bg-[rgba(255,107,0,0.12)]'}`}>{opt.label}</button>)}</div>
